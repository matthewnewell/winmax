...
_DEPOT_API_URL = os.environ.get("DEPOT_API_URL", "http://localhost:8090").rstrip("/")
...
def _depot(messages: list[dict], system: str, max_tokens: int = 1024) -> str:
    ...
    AI_API_KEY of its own, only DEPOT_API_URL. Same error/degrade shape as every other provider
    function: a string starting "[AI error: ...]" or a plain reachability message, never an
    exception."""
    try:
        import httpx
    except ImportError:
    ...
    try:
        ...
        return f"Could not reach the Depot's AI proxy at {_DEPOT_API_URL}. Is it running?"
    except Exception as e:
        return f"[AI error: {e}]"
